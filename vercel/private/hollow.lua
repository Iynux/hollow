...
getgenv().HollowWindowIcon = WindowIcon
...
getgenv().HollowRaidSummon = {
    start = startRaidSummonSpam,
    stop = stopRaidSummonSpam,
    restore = restoreRaidSummonToggles,
    stopAll = stopAllRaidSummonSpam,
}
...
local function pickInitialLobbyPad(mapDefOrKeys, entry, lobbyCFrame, useRandomLobby)
    if useRandomLobby then
        local randomPos = getRandomLobbyTeleportPos()
        if randomPos then
            return randomPos
        end
    end
...
local function fireGlobalInitQuickstart(mapKey, gamemode, mapDefOrKeys)
    ...
    local function fireRemote(name, ...)
        ...
        if not ok then
            if #args > 0 then
                fireLobbyRemote(name, table.unpack(args))
            else
                fireLobbyRemote(name)
            end
        end
...
getgenv().JoinMapHard = function(mapDefOrKeys, lobbyCFrame, gamemode)
    ...
    local ok, err = pcall(function()
        ...
        local useRandomLobby = true
        if type(mapDefOrKeys) == "table" and mapDefOrKeys.randomLobby == false then
            useRandomLobby = false
        end
        local GlobalInit = getGlobalInitEvents()
        if not GlobalInit then
            return
...
local function bindFileToggle(toggleName, fileKey, onEnable)
    Toggles[toggleName]:OnChanged(function(value)
        ...
        if not loadingSettings and not getgenv().HollowLoadingSettings then
            saveToggleState(fileKey, value)
        end

        if value and onEnable and not loadingSettings and not getgenv().HollowLoadingSettings then
            task.spawn(onEnable)
        end
    end)
end
...
getgenv().HollowCore = {
    ...
    runScriptModule = runScriptModule,
    runAutoMap = runAutoMap,
    bindFileToggle = bindFileToggle,
    ...
    styleNeverloseRowControls = styleNeverloseRowControls,
    styleNeverloseTextBoxes = styleNeverloseTextBoxes,
}
end

do
local Core = getgenv().HollowCore
local fetchModule = Core.fetchModule
local loadModule = Core.loadModule
local runScriptModule = Core.runScriptModule
local runAutoMap = Core.runAutoMap
local bindFileToggle = Core.bindFileToggle
local readToggle = Core.readToggle
...
local saveAllSettings = Core.saveAllSettings
local styleNeverloseRowControls = Core.styleNeverloseRowControls
local styleNeverloseTextBoxes = Core.styleNeverloseTextBoxes
...
local runAutoSummon, runAutoFish, runAutoEmilia, runAutoBounty, exchangeRubies, duplicateUnitByName, autoInputTowers, queueAutoSave, hookAutoSave, loadSavedSettings, ensureHollowFolder, syncTowerAliases, onLoadoutDisabled, startHotbarAutoSync
    ...
    for toggleName, toggle in pairs(Toggles or {}) do
        ...
        if not hooked then
            ...
            toggle:OnChanged(function(value)
                if getgenv().HollowLoadingSettings then
                    return
                end
...
restoreEnabledFeatures = function()
    ...
    getgenv().HollowLoadingSettings = true
    ...
    getgenv().HollowLoadingSettings = false
...
getgenv().HollowRuntime = {
    runAutoSummon = runAutoSummon,
    runAutoFish = runAutoFish,
    runAutoEmilia = runAutoEmilia,
    runAutoBounty = runAutoBounty,
    exchangeRubies = exchangeRubies,
    duplicateUnitByName = duplicateUnitByName,
    autoInputTowers = autoInputTowers,
    loadSavedSettings = loadSavedSettings,
    hookAutoSave = hookAutoSave,
    syncTowerAliases = syncTowerAliases,
    onLoadoutDisabled = onLoadoutDisabled,
    restoreEnabledFeatures = restoreEnabledFeatures,
    startHotbarAutoSync = startHotbarAutoSync,
    ensureHollowFolder = ensureHollowFolder,
}

end

getgenv().HollowUIBoot = {
    Toggles = Toggles,
    Options = Options,
    LocalPlayer = LocalPlayer,
    mapToggleDefs = mapToggleDefs,
    RAID_SUMMON_DEFS = RAID_SUMMON_DEFS,
    TOWER_ID_UI_NAMES = TOWER_ID_UI_NAMES,
    LOADOUT_NAMES = LOADOUT_NAMES,
    DUPE_REMOTE_CACHE = "Hollow/dupe_remote.txt",
    DUPE_PAYLOAD_CACHE = "Hollow/dupe_payload.txt",
    DUPE_UNIT_CACHE = "Hollow/dupe_unit.txt",
    DUPE_COUNT_CACHE = "Hollow/dupe_count.txt",
    DUPE_SLOT_COUNT = 6,
    applyWindowBackground = applyWindowBackground,
    fixWindowHeaderLayout = fixWindowHeaderLayout,
}

local bootSource = ScriptModules and ScriptModules["HollowBoot.lua"]
if not bootSource then
    hollowStartupError("Hollow", "Missing HollowBoot.lua module.")
    return
end
local bootFn, bootErr = loadstring(bootSource, "HollowBoot")
if not bootFn then
    hollowStartupError("Hollow", "Failed to parse UI boot: " .. tostring(bootErr))
    return
end
local bootOk, bootRunErr = pcall(bootFn)
if not bootOk then
    hollowStartupError("Hollow", "UI boot failed: " .. tostring(bootRunErr))
    return
end
